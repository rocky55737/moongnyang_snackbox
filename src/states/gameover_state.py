...
class GameOverState(BaseState):
    def on_enter(self) -> None:
        self.made_max = any(s.tier == MAX_TIER for s in self.game.world.snacks)
    ...
    def draw(self, surface) -> None:
        ...
        title = "낙곱새 완성! 대성공!" if self.made_max else "간식 상자가 꽉 찼어요!"
        r.draw_text(surface, title, 25, (255, 158, 199), center=(WIDTH // 2, HEIGHT // 2 - 42))
        r.draw_text(surface, f"점수 {self.game.score.score}", 30, (255, 215, 106),
                    center=(WIDTH // 2, HEIGHT // 2 + 4))
        r.draw_text(surface, "클릭 또는 아무 키로 다시하기", 16, (220, 210, 255),
                    center=(WIDTH // 2, HEIGHT // 2 + 48))
